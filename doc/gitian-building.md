...
*Setup instructions for a Gitian build of Phloxcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Phloxcoin
...
- [Building Phloxcoin Core](#building-phloxcoin-core)
...
Clone the git repositories for phloxcoin and Gitian.
...
in the phloxcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Phloxcoin Core
----------------

To build Phloxcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the phloxcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/phloxcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/phloxcoin-project/phloxcoin
...
For example:
```bash
URL=https://github.com/thrasher-/phloxcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit phloxcoin=${COMMIT} --url phloxcoin=${URL} ../phloxcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit phloxcoin=${COMMIT} --url phloxcoin=${URL} ../phloxcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit phloxcoin=${COMMIT} --url phloxcoin=${URL} ../phloxcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the phloxcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../phloxcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/phloxcoin
SIGPATH=/some/root/path/phloxcoin-detached-sigs

./bin/gbuild --url phloxcoin=${BTCPATH},signature=${SIGPATH} ../phloxcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/phloxcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/phloxcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/phloxcoin-osx-build.assert
```
...
[phloxcoin-project/gitian.sigs.ltc](https://github.com/phloxcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
