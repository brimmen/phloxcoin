...
#if defined(NDEBUG)
# error "Phloxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2361ddda9d1d5e2f601f3b672e5baead4840cabfd097fa92491822459725ab98");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Phloxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Phloxcoin Signed Message:\n";
...
{
    // Phloxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Phloxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Phloxcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Phloxcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Phloxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Phloxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2abeb56c34b670f21691c5931fcebbf372a7c2448b7e7a8d1225222f5f23a7f1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04beb92baa70e4df202395e4fa54fb251b018fbeec447821afdf6374a96b53ce69f418a154f5f17ba079cff80aa0356a0bf450ea7d4b2d56a9ec74a06fa868f398") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1683866301;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084662826;

        if (fTestNet)
        {
            block.nTime    = 1683866285;
            block.nNonce   = 385405023;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x111f999cfea7857f0174694b44999f5b547e3db8f0717c8f9e5533743dcbdb00"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Phloxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PhloxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PhloxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PhloxcoinMiner(CWallet *pwallet)
{
    printf("PhloxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("phloxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PhloxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PhloxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PhloxcoinMiner, pwallet));
}
